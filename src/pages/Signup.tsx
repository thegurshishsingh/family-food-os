    ...
    <>
      ...
      <div className="hidden md:flex min-h-screen items-center justify-center bg-background px-4">
        <div className="w-full max-w-sm">
          ...
          <Button variant="outline" className="w-full mb-6 h-11" onClick={handleGoogleSignup}>
            {googleIcon}
      ...
      <div className="block md:hidden min-h-screen bg-background px-6 py-8 safe-top safe-bottom flex flex-col">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">Start planning your family's real week of food</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4 flex-1">
          ...
          <div>
            ...
            <Input id="password-m" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required minLength={6} className="mt-1.5 h-[50px]" />
          </div>
          <Button type="submit" className="w-full h-14 text-base font-semibold rounded-xl" disabled={loading}>
            {loading ? "Creating account..." : "Create account"}
          </Button>

          <Button variant="outline" className="w-full h-14 text-base rounded-xl" type="button" onClick={handleGoogleSignup}>
            {googleIcon}
            Continue with Google
          </Button>
        </form>
