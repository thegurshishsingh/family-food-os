...
function WeeklyPlanCard({ isMobile, viewport }: { isMobile: boolean; viewport: { once: boolean; amount: number } }) {
  return (
    <div className="rounded-2xl glass-strong overflow-hidden flex flex-col w-full min-h-[406px] shadow-xl">
      ...
      <div className="flex-1">
        {WEEK_PREVIEW.map((day, i) => (
          <motion.div
            key={day.day}
            className="flex items-center gap-4 px-6 py-3 border-b border-border/20 last:border-b-0 group transition-colors hover:bg-primary/3"
